...
function getDimensionUnit(args) {
    const selectedFlags = [
        { flag: "-ub", unit: "b" },
        { flag: "-uk", unit: "k" },
        { flag: "-um", unit: "m" },
...
async function main() {
    ...
    if (tIndex === -1 || !STRATEGIES[args[tIndex + 1]]) {
        ...
        console.error(`Usage: movx -t <${Object.keys(STRATEGIES).sort().join("|")}>`);
    ...
    let strategy;
    try {
        strategy = (await Promise.resolve(`${STRATEGIES[mode]}`).then(s => __importStar(require(s)))).default;
    }
    catch (err) {
    ...
    for (const filePath of allFiles) {
        const targetDirName = await strategy.getTargetDir(filePath, strategyOptions);
        if (!targetDirName)
            continue;
        const fileName = path_1.default.basename(filePath);
        const targetDir = path_1.default.join(currentDir, targetDirName);
        if (!fs_1.default.existsSync(targetDir)) {
            fs_1.default.mkdirSync(targetDir, { recursive: true });
        }
        let targetPath = path_1.default.join(targetDir, fileName);
        ...
        while (fs_1.default.existsSync(targetPath)) {
            targetPath = path_1.default.join(targetDir, `${nameWithoutExt}_${counter}${ext}`);
            counter++;
        }
        try {
            fs_1.default.renameSync(filePath, targetPath);
            console.log(`Moved: ${path_1.default.relative(currentDir, filePath)} -> ${path_1.default.relative(currentDir, targetPath)}`);
            movedCount++;
        }
        catch (err) {
            const message = err instanceof Error ? err.message : String(err);
